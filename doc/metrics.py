...
class MetricsTracker:
    ...
    def get_best_metric(self, metric_name: str, minimize: bool = True) -> tuple[int, float]:
        ...
        values = [
            (epoch, metrics.get(metric_name, float("inf") if minimize else float("-inf")))
...
def compute_text_detection_metrics(
    pred_binary: torch.Tensor, gt_text: torch.Tensor, threshold: float = 0.5
) -> dict[str, float]:
    """
