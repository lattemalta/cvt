...
class DBNetLoss(nn.Module):
    ...
    def __init__(self, 
                 ...
                 gamma: float = 1.0): # Weight for threshold map loss
        super().__init__()
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
    ...
    def _create_threshold_target_and_mask(self, 
                                        gt_text: torch.Tensor, 
                                        shrink_ratio: float = 0.4) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        for i in range(batch_size):
            ...
            if text_regions.sum() > 0:
                ...
                threshold_map[text_regions > 0] = distance_inside[text_regions > 0] / (distance_inside[text_regions > 0].max() + 1e-6)
    ...
    def forward(self, 
        ...
        # Combined loss
        total_loss = (self.alpha * prob_loss + 
                     self.beta * binary_loss + 
                     self.gamma * thresh_loss)
