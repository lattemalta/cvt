...
class FeaturePyramidNetwork(nn.Module):
    def __init__(self, in_channels: list[int], out_channels: int = 256):
        ...
        self.lateral_convs = nn.ModuleList(
            [nn.Conv2d(in_channel, out_channels, kernel_size=1, bias=False) for in_channel in in_channels]
        )

        # Output convolutions (3x3 conv for smoothing)
        self.fpn_convs = nn.ModuleList(
            [nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, bias=False) for _ in in_channels]
        )

        # Batch normalization layers
        self.lateral_bns = nn.ModuleList([nn.BatchNorm2d(out_channels) for _ in in_channels])

        self.fpn_bns = nn.ModuleList([nn.BatchNorm2d(out_channels) for _ in in_channels])
    ...
    def forward(self, features: list[torch.Tensor]) -> list[torch.Tensor]:
        ...
        for i in range(len(laterals) - 2, -1, -1):
            ...
            upsampled = F.interpolate(
                fpn_features[-1], size=laterals[i].shape[2:], mode="bilinear", align_corners=False
            )
